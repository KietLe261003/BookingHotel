...
const CreateHotelForm = ({ getAllHotels }) => {
  const { isOpen, onOpen, onClose } = useDisclosure();

    const createHotel = async (data) => {
      try {
        await hotelServices.createHotel(data);
        notification.success({ message: "Tạo khách sạn thành công" });
        onClose();
        getAllHotels();
      } catch (error) {
        notification.error({ message: "Lỗi tạo khách sạn" });
      }
    };
  ...
  return (
    <div className="flex justify-end m-3">
      <Button onClick={onOpen} className="text-right bg-white">
        Tạo khách sạn
      </Button>
      ...
      <Modal onClose={onClose} isOpen={isOpen} size={"3xl"} isCentered>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Tạo khách sạn</ModalHeader>
          <ModalCloseButton />
